...
public class BulkApiController implements BulkApi {
    ...
    private final BulkApiDelegate delegate;

    public BulkApiController(@Autowired(required = false) BulkApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new BulkApiDelegate() {});
    }

    @Override
    public BulkApiDelegate getDelegate() {
        return delegate;
    }
